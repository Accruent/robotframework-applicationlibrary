...
try:
    ...
    AppiumCommon = importlib.import_module('.Helpers.AppiumCommon', package='ApplicationLibrary')
...
class DesktopLibrary(AppiumLibrary):
    """ApplicationLibrary Desktop Library
    ...
    def send_keys(self, *argv):
        ...
        if argv:
            ...
            appLib.fail('No key arguments specified.')
    ...
    def _element_find(self, locator, first_only, *kwargs):
        ...
        if prefix == 'image':
            try:
                ...
                appLib.fail("Selecting by image is only available when using Appium "
                            "v1.18.0 or higher")
        appLib.fail("Element locator with prefix '" + prefix + "' is not supported")
    ...
    def _is_element_present(self, locator):
        ...
        if prefix is None:
            ...
            try:
                ...
                appLib.fail("Selecting by image is only available when using Appium v1.18.0 or higher")
        else:
            appLib.fail("Element locator with prefix '" + prefix + "' is not supported")
